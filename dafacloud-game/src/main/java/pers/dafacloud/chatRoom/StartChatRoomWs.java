...
public class StartChatRoomWs {
    ...
    public static void main(String[] args) throws Exception {
        ...
        Header[] headers = HttpHeader.custom()
                ...
                .build();
        List<String> users = FileUtil.readFile(StartChatRoomWs.class.getResourceAsStream("/test-testChatRoom.txt"));
        for (int i = 0; i < 100; i++) {
            int ii = i;
            new Thread(() -> {
                ...
                ChatRoomResponce chatRoomResponce = new ChatRoomResponce(users.get(ii));
                try {
                    ...
                    for (int j = 0; j < 10000000; j++) {
                        ...
                        int index = 5 + (int) (Math.random() * 10);
                        Thread.sleep(index * 1000);
                        //roomCode=00002&messageTypeId=1&status=0&userIds=&message=czxcxz
                        httpConfig.body("roomCode=00002&messageTypeId=1&status=0&userIds=&message=dukeTestdukeTestdukeTestdukedukeTestdukeTestdukeTestdukedukeTestdukeTestdukeTestduke")
                                .url("http://" + host + "/v1/chat/addMessage");
                        System.out.println(DafaRequest.post(httpConfig));
                        if (j % 3 == 0) {
