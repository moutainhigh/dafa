...
public class SendMessageSX {
    ...
    private void getToken() {
        ...
        createConnection(url.replaceAll("(TOKEN=).*?(&gameId)", String.format("$1%s$2", token)), httpConfig);
    }

    /**
     * 1.创建链接
     */
    private void createConnection(String url, HttpConfig httpConfig) {
        try {
    ...
    public Map<String, String> process() {
        try {
            //WebSocketContainer container = ContainerProvider.getWebSocketContainer();
            //ResponceMessage responceMessage = new ResponceMessage(username);//返回信息类
            //session = container.connectToServer(responceMessage, URI.create(url));
            //session.setMaxTextMessageBufferSize(2048000);//设置缓冲文本大小
            //session.setMaxBinaryMessageBufferSize(204800);
            for (int i = 0; i < 70000000; i++) {
                ...
                //System.out.println("----"+ResponceMessage.count);
                if (session.isOpen()) {
                    if (ResponceMessage.count == 13) {
                        ...
                        } else if (ResponceMessage.chipTotalPos1 < ResponceMessage.chipTotalPos2) { //下注小的一边
                            if ("duke002".equals(username)) {
                                betGameContent.setPos("2");
                                session.getAsyncRemote().sendText(betGameContent.toString());
                            } else if ("duke003".equals(username)) {
                    ...
                    ////"ws://" + new URL(host).getHost() + ":4147/gameServer/?TOKEN=" + token + "&gameId=" + gameCode
                    //session = container.connectToServer(responceMessage, URI.create(url));
                    //session.setMaxTextMessageBufferSize(2048000);//设置缓冲文本大小
                    //session.setMaxBinaryMessageBufferSize(204800);
                    getToken();
