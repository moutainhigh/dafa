...
public class Dafa1F5FTestUrl implements SchedulingConfigurer {
    ...
    private static List<String> betUser =
            FileUtil.readFile(Dafa1F5FTestUrl.class.getResourceAsStream("/user0.txt"));
    ...
    private static String[] lotteryCodes1f = {"1300"};
    ...
    public static void init()  {
        ...
        for (int i = 0; i < 8; i++) {
            ...
            excutors0.execute(() -> {
                ...
                String body = getLoginBody(betUser.get(attri), "123456");
        ...
        httpConfigSplit = ListSplit.split(httpConfigs, 4);//拆分
        System.out.println(httpConfigSplit.size());
        //finishlLogin = true;
    }
    ...
    public void aa() {
        ...
        if (listnew5fen.size() != 0)
            splitTask(httpConfigSplit.get(1), listnew5fen, issueCurrent, "2");
    }
    ...
    public static void splitTask(List<HttpConfig> httpConfigs, List<List<GetBetInfo>> listnew, String issueCurrent, String from) {
        ...
        for (int i = 0; i < listnew.size(); i++) { //
            ...
            excutors.execute(() -> {
                ...
                bet(httpConfigs.get(attri), listnew.get(attri).subList(0, 17), issueCurrent);
