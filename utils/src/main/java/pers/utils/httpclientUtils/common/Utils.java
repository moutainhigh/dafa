package pers.utils.httpclientUtils.common;
...
private final static Logger logger = LoggerFactory.getLogger(pers.utils.logUtils.Log.class);
...
//							builder.addPart(String.valueOf(map.get(ENTITY_MULTIPART+".name")) + count++, fileBody);
					...
					nvps.add(new BasicNameValuePair(entry.getKey(), String.valueOf(entry.getValue()))); //正常map
				}
			}
			if(!isSpecial) {
