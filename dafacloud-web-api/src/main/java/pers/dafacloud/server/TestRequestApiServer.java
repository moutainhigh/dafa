...
public class TestRequestApiServer {
    ...
    private TestApiResult testApiResult;
    ...
    public Response testApiOne(int id, String host, String cookie) {
        ...
        //---------------------------------------------- 测试结果----------------------------------------------
        testApiResult = new TestApiResult();
        testApiResult.setHost(httpHost);
        ...
        if (StringUtils.isNotEmpty(dependentApi)) {//依赖不为空
            try {
                requestParametersNew = deRequestHandle(httpHost, cookie, httpConfig, testApiResult, dependentApi, apiManage.getRequestParameters());
            } catch (Exception e) {
        ...
        try {
            String result = requestHandle(apiManage, httpHost, cookie, httpConfig, testApiResult, false);//正式请求返回
     ...
     * 批量运行task
     */
    private void task(String host, String cookie, String testBatch, List<ApiManage> apiManages) throws Exception {
     ...
     * @return Header[] 请求头
     */
    private static Header[] headerHandle(String cookie, String requestHeader) throws Exception {
    ...
    private static String requestParametersHandle(String requestParameters, String url, boolean isGet) throws Exception {
        ...
        return isGet ? urlBuilder.url(url).fullUrl() : urlBuilder.fullBody();
    }
     ...
     */
    private String requestHandle(ApiManage apiManage, String httpHost, String cookie, HttpConfig httpConfig, TestApiResult testApiResult, boolean isDependent) throws Exception {
        // ------------------------------请求头----------------------------------------------
        Header[] headers = headerHandle(cookie, apiManage.getRequestHeader());
        httpConfig.headers(headers);
        // ------------------------------请求参数----------------------------------------------
        String requestParameters = requestParametersHandle(apiManage.getRequestParameters(), httpHost + apiManage.getPath(), apiManage.getMethod() == 1);
        // ------------------------------请求发送----------------------------------------------
        String result = apiManage.getMethod() == 1 ? DafaRequest.get(httpConfig.url(requestParameters))
                : DafaRequest.post(httpConfig.url(httpHost + apiManage.getPath()).body(requestParameters));
        if (!isDependent) {
