...
public class ThreadTenantLotteryPre {
    ...
    public static void main(String[] args) throws Exception {
        List<String> betContents1k = FileUtil.
                readFile(ThreadTenantLotteryPre.class.getResourceAsStream("/svBetContent/1418.txt"));
        List<String> betContents5k = FileUtil.
                readFile(ThreadTenantLotteryPre.class.getResourceAsStream("/svBetContent/1419.txt"));
        List<String> betContents1s = FileUtil.
                readFile(ThreadTenantLotteryPre.class.getResourceAsStream("/svBetContent/1018.txt"));
        List<String> betContents5s = FileUtil.
                readFile(ThreadTenantLotteryPre.class.getResourceAsStream("/svBetContent/1019.txt"));
    ...
    public static void betting(List<String> users1, List<String> betContents1, int sleepSecond1) {
        for (int i = 0; i < users1.size(); i++) { //用户
            ...
            new Thread(() -> {
                ...
                HttpConfig httpConfig = HttpConfig.custom().url(loginUrl).body(body).headers(headers).cookieStore(new BasicCookieStore());
                String result0 = DafaRequest.post(httpConfig);
                System.out.println(users.get(index)+"，"+result0);
                //String JSESSIONID = JSONObject.fromObject(result0).getString("data");
                ////登录
                ...
                //httpConfig = httpConfig.context(context);
