...
public class ThreadTenantLottery {
    ...
    public static void main(String[] args) throws Exception {
        List<String> betContents = FileUtil.readFile(ThreadTenantLottery.class.getResourceAsStream("/svBetContent/1418distinct.txt"));//1分快3投注内容
        String path = "/sv01";
        List<String> user005300 = FileUtil.readFile(ThreadTenantLottery.class.getResourceAsStream(path + "/005300.txt"));
        List<String> user006200 = FileUtil.readFile(ThreadTenantLottery.class.getResourceAsStream(path + "/006200.txt"));
        List<String> user007100 = FileUtil.readFile(ThreadTenantLottery.class.getResourceAsStream(path + "/007100.txt"));

        betting(user005300, betContents, 1);//005站 300

        betting(user006200, betContents, 1);//006站 200

        betting(user007100, betContents, 1);//007站 100

        //List<String> user008 = new ArrayList<>();
        //user008.add("50360170,shalv008,shalv008a0099");
        //betting(user008, betContents, 120);//008站 1

    }

    public static void betting(List<String> users, List<String> betContents, int sleepSecond) {
        for (int i = 0; i < users.size(); i++) { //用户
            int index = i;
            new Thread(() -> {
